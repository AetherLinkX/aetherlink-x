...
import com.palazik.vpn.compat.Uri
...
object ProfileCodec {
    ...
    fun decode(raw: String): VpnProfile? = runCatching {
        ...
        when {
            ...
            // plain http:// is ambiguous (it's how subscription URLs look), so an
    ...
    fun decodeSubscriptionBody(body: String): List<VpnProfile> {
        ...
        // A plain-text body can still be "Base64-decodable" into garbage, which
        // would yield zero profiles. Fall back to parsing the original lines.
    ...
    fun encodePalazik(p: VpnProfile): String {
        val json = JSONObject().apply {
            put("v", 1)
            put("id", p.id)   // persist id so active-profile state survives restart
    ...
    private fun decodePalazik(raw: String): VpnProfile {
        ...
        return VpnProfile(
            id          = savedId ?: UUID.randomUUID().toString(),
            protocol    = protocol,
    ...
    private fun decodeTrojan(raw: String): VpnProfile {
        val uri    = Uri.parse(raw)
        val params = uri.queryParameterNames.associateWith { uri.getQueryParameter(it) ?: "" }
        // Trojan can run over WS/gRPC/H2 — read type/path/host so non-TCP
