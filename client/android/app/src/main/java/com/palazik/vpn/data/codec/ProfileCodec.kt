...
object ProfileCodec {
    ...
    private fun decodeVnextXrayOutbounds(outbound: JSONObject, remark: String): List<VpnProfile> {
        val baseProtocol = when (outbound.optString("protocol").lowercase()) {
            "vless" -> Protocol.VLESS
            "vmess" -> Protocol.VMESS
            else -> return emptyList()
        }
        val protocol = if (
            baseProtocol == Protocol.VLESS && remark.contains("AetherLink X", ignoreCase = true)
        ) Protocol.AETHERLINK_X else baseProtocol
    ...
    private fun decodeVless(raw: String): VpnProfile {
        ...
        val name = Uri.decode(uri.fragment ?: "VLESS")
        return VpnProfile(
            name        = name,
            protocol    = if (name.contains("AetherLink X", ignoreCase = true)) Protocol.AETHERLINK_X else Protocol.VLESS,
            address     = uri.host ?: "",
    ...
    private fun encodeAetherLinkNative(p: VpnProfile): String =
        Uri.Builder().scheme("aetherlink")
            ...
            .appendQueryParameter("mode", p.transportMode.ifBlank { "tcp-first" })
            .fragment(p.name)
            .build()
            .toString()
