...
- [heiher/hev-socks5-tunnel](https://github.com/heiher/hev-socks5-tunnel) — MIT-licensed мост TUN-to-SOCKS, используемый в рабочем тракте Android. Сборка закреплена на конкретном commit и выполняется из исходников в CI.
...
4. `palazikVpnService` строит full-device TUN, а `hev-socks5-tunnel` переводит его IP-пакеты в TCP/UDP SOCKS5.
5. Пропатченный Xray через `libv2ray` слушает приватный динамический SOCKS-порт и выполняет выбранный outbound, включая AetherLink X. Фиксированный `10808` не используется.
6. DNS IPv4/IPv6 захватываются TUN-интерфейсом и идут через тот же SOCKS/Xray тракт; приложение исключается из петли.
...
перехватываются Android TUN и передаются через HEV в SOCKS inbound Xray, а AetherLink X кодирует
