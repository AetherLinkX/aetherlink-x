...
- [heiher/hev-socks5-tunnel](https://github.com/heiher/hev-socks5-tunnel) был исследован как альтернативный мост; после диагностики рабочий тракт переведён на встроенный TUN Xray, чтобы убрать лишнее копирование пакетов.
...
4. `palazikVpnService` строит full-device TUN и передаёт его дескриптор непосредственно пропатченному Xray через `libv2ray`.
5. Xray также слушает приватный динамический SOCKS-порт только для внутренних end-to-end проверок. Фиксированный `10808` не используется в боевом VPN-сеансе.
...
перехватываются нативным TUN Xray, а AetherLink X кодирует
...
Xray loop и старый TUN, затем запускает новый профиль внутри того же
